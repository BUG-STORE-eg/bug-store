...
function addToCart(name,price){cart.push({name,price});save();openCart()}
function openCart(){const m=document.getElementById('modal');if(m){m.classList.add('show');renderCart()}}
function closeCart(){const m=document.getElementById('modal');if(m)m.classList.remove('show')}
function renderCart(){const box=document.getElementById('items'),t=document.getElementById('total');if(!box)return;let total=0;box.innerHTML=cart.length?cart.map((x,i)=>{total+=x.price;return `<div class="row"><span>${x.name}</span><span>${x.price} جنيه <button onclick="removeItem(${i})">حذف</button></span></div>`}).join(''):'<p class="muted">السلة فارغة.</p>';if(t)t.textContent=total}
function removeItem(i){cart.splice(i,1);save();renderCart()}
function checkout(){if(!cart.length)return;let total=0,msg='مرحباً BUG STORE، أريد طلب:\n';cart.forEach(x=>{total+=x.price;msg+=`- ${x.name}: ${x.price} جنيه\n`});msg+=`\nالإجمالي: ${total} جنيه`;window.open(`https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(msg)}`,'_blank')}
function quickOrder(name,price){addToCart(name,price)}
function renderGame(id){const g=games[id],t=document.getElementById('gameTitle'),d=document.getElementById('gameDesc'),p=document.getElementById('prices');if(!g||!p)return;t.textContent=g.name;d.textContent=g.desc;document.querySelectorAll('.game-tab').forEach(b=>b.classList.toggle('active',b.dataset.game===id));p.innerHTML=g.items.map(x=>`<article class="price-card"><div class="coin">🎮</div><h3>${x[0]}</h3><span class="muted">شحن سريع</span><strong>${x[1]} جنيه</strong><button onclick="addToCart('${g.name} - ${x[0]}',${x[1]})">اطلب الآن 🛒</button></article>`).join('')}
document.addEventListener('DOMContentLoaded',()=>{save();document.querySelectorAll('.game-tab').forEach(b=>b.addEventListener('click',()=>renderGame(b.dataset.game)));if(document.getElementById('prices'))renderGame('pubg')});
